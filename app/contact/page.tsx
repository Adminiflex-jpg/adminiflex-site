...
export default function ContactPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 md:px-6 py-16">
      ...
      <p className="mt-2 text-zinc-700">Stel je vraag of plan een demo. We reageren doorgaans binnen één werkdag.</p>

      <form onSubmit={onSubmit} className="mt-8 grid gap-4">
        {/* Honeypot veld - verstoppen voor bots */}
        ...
        {status && (
          <div className={`text-sm ${status.ok ? "text-emerald-700" : "text-red-600"}`}>{status.msg}</div>
        )}
      </form>
    </main>
  );
